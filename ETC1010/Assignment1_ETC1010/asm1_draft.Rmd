...
crime_dat <- read_csv("C:/Users/sjsa3/Desktop/Shared_with_Mac/year2_sem1/ETC1010/Assignment1_ETC1010/SuburbData2019.csv")
```

```{r}
# I am selecting here only a portion of the data
# to reduce computation times.
##!Note : Since from the question 20 onwards needs the data whose suburban named "Coogee", therefore, I added a new "Coogee"!
...
```{r crime-year-summary-suburb, eval = FALSE}
## !Not sure which dataset!
...
```{r crime-offence-paddington, eval = FALSE}
...
crime_long_new %>% 
  ...
  mutate(Year = as.numeric(Year)) %>%  # 1pt
  ggplot(aes(x = Year, y = Number_of_incidents )) +  # 1pt
  geom_line(linetype = "dashed", color ="Green")  # 1pt
```
...
creating a bar plot where we have the incidents per suburb by year next to each other
```{r , eval = FALSE}
...
ggplot(comparison_data, aes(Number_of_incidents  ,    # 1pt
                            # Y =Year ,  # 1pt
                            fill = Suburb)) +  # 1pt
  geom_bar(stat = "identity",   # 1pt 
