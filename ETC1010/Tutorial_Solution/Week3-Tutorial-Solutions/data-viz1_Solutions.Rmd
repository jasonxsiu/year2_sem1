...
```{r month-label-abbr}
month("2020-01-20", label = TRUE, abbr  = FALSE)
```
...
> The distribution of pedestrian counts seem to overlap with each other, given the 2 boxes lie so close to each other. Flinder's Underpass has a higher median pedestrian Counts and contain outliers as compared to Melbourne Central (no outliers). Need more information/in-depth analysis to draw insights from the data!
...
> Time Series graph from pedestrian counts in Melbourne Central Station stay the same throughout the first week of March. The pattern at Flinders Street Station however, is different for Weekdays and Weekends. The weekends lack the two-distinct peaks corresponding to morning and evening rush-hours.
...
Next we save the data into a (compressed) CSV format. You can run this code chunk below ("read-data") if you want to read the data from the "data" folder. **Remember that if you knit this document, you will need to set the code chunk to `eval = TRUE`.**
