...
```{r lm-day}
co2_day_fit <- ___(co2 ~ ___, data = ___)
___(___)
```
...
- The variables, day and day0 range from  ___ to ___


### Q5. Fit a linear model, predicting co2 using day0

```{r fit-co2}
co2_fit <- ___(___ ~ ___, data = ___)
co2_fit
```
...
```{r daysq-mod-plot}
co2_model2 <- ___(co2_fit2, co2_sq)

ggplot(___, 
       aes(x = ___, y = ___)) + 
  geom_point() + 
  geom_line(aes(y = ___), colour="blue")
```
...
```{r read-ptb}
co2_ptb <- read_csv(
  file = here::here("Week7-Workshop2/data/daily_merge_co2_spo.csv"),
                    col_names = c("date", 
                                  "time", 
                                  "day", 
                                  "decdate", 
