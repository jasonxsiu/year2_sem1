---
title: "Exercise 1: Intro to data modeling"
author: "your name"
date: "`r Sys.Date()`"
...
# Section A: Practice with Paintings dataset
...
The formula `response ~ explanatory` specifies the response variable and explanatory variable from the data. In this case the response is height 
...
The function `tidy()` will extract the estimated coefficients as a tidy table.
...
Yet, another and a better way to visualise residuals is using `resid_panel`
```{r}
library(ggResidpanel)
resid_panel(mod_heights, plots = "all")
```
...
# Section B: Your turn with Categorical Predictor variable

Construct a linear model of heights as a function of one of the categorical genre variables, say `still_life`. Go through the process of interpretation, and checking the fit. Remember to set the variable to be a factor
before running the model. 

### Q1. Construct a linear model of heights as a function of one of the categorical genre variables, say `still_life`.
```{r mod-still-life}
paintings <- ___ %>% 
  mutate(___ = factor(___))

mod_heights_sl <- __(height_in ~ ___, data = ___)
___(mod_heights_sl)
```

### Q2. How does it compare to the model we created above?
```{r model-glance-bind}
mod_heights_sl_summary <- ___(mod_heights_sl)
...
where $\varepsilon_i$ is distributed according to a normal distribution, $N(0, \sigma^2)$.

### Q3. From the plot below, answer the questions
