...
```{r}
pp_lm_aug %>% ggplot(aes(x = .fitted, y = .resid)) +
   geom_point()

pp_rp_aug %>% ggplot(aes(x = .fitted, y = .resid)) +
   geom_point()


# also, look at fitted vs predicted
pp_lm_aug %>% ggplot(aes(x = .fitted, y = logprice)) +
   geom_point()

pp_rp_aug %>% ggplot(aes(x = .fitted, y = logprice)) +
   geom_point()
...
pp_rp_simple <- rpart(logprice ~ origin_author + school_pntg + mat , data = pp)
...
```{r}
new_painting <- tibble(origin_author = "X", school_pntg = "X", mat = "c")

augment(pp_rp_simple, newdata = new_painting)

# You could use this plot to predict too:
rpart.plot(pp_rp_simple)
...
```{r}
pp_lm_simple <- lm(logprice ~ origin_author + school_pntg + mat , data = pp)

augment(pp_lm_simple, newdata = new_painting)
